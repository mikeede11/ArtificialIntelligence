...
def create(x):# x will be game board [5,6,3,4,2,1,0,...]
    s=stack.create(x)
    return [ s,1,x,False, 0 ] #there is one initial state from create in the frontier

def is_empty(s): #stak represents the moves we can do
    return stack.is_empty(s[0]) and not s[3] # stack is empty and try next level is false

def insert(s,x):# s is frontier
