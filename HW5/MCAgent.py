...
def MCAgent(board, color):
    ...
    for col in valid_locations:
        ...
        current_wins = 0
    return max(col_prob, key=col_prob.get)
...
def play_game(boardCopy, color):
    turn = 0 #make sure the other guy goes "1st"
    other_color = None
    if color == BLUE_INT:
        other_color = RED_INT
    else:
        other_color = BLUE_INT
    # board = create_board()
    # print_board(board)
    game_over = False
    ...
    while not game_over:
        if turn % 2 == 0:#PLAYER 1 ME / MY AGENT
            MoveRandom(boardCopy,other_color)

        if turn % 2 == 1 and not game_over:#PLAYER 2 RANDOM AGENT
            MoveRandom(boardCopy, color) #EXECUTE MOVE

        # print_board(boardCopy) #PRINT UPDATE
        
        #CHECK GAME STATUS AFTER EACH MOVE
        if game_is_won(boardCopy, color): 
            game_over = True
            return 1
            # agent1Wins += 1
            # print(colored("Red wins!", 'red'))
        if game_is_won(boardCopy, other_color):
            game_over = True
            return 0
            # randomAgentWins += 1
            # print(colored("Blue wins!", 'blue'))
        if len(get_valid_locations(boardCopy)) == 0:
            game_over = True
            return 0
...
print("our Agents win Rate was ", mcWins/10, " % ")