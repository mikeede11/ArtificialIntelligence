...
def hdistance2(s): #This will be the Manhattan distance heuristic
    ...
    for i in range(len(s[0])):
        totalManhattanDistance += manhattanDistance(i, n, s[0][i])
    return totalManhattanDistance

def manhattanDistance(index, n, tileVal):
    if(tileVal == 0):
        return 0
    currentCol = int(index % n)
    currentRow = int(index / n)
    goalCol = int(tileVal % n)
    goalRow = int(tileVal / n)
    return abs(currentCol - goalCol) + abs(currentRow - goalRow)